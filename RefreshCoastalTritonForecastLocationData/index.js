...
module.exports = async function (context, message) {
  // Location of csv:
  const csvUrl = process.env['COASTAL_TRITON_WORKFLOW_URL']
  // Destination table in staging database
  const tableName = 'COASTAL_FORECAST_LOCATION'
  const partialTableUpdate = { flag: true, whereClause: `where COASTAL_TYPE = 'Triton'` }
  ...
  if (failedRows.length > 0) {
    await doInTransaction(loadExceptions, context, 'The triton coastal location exception load has failed with the following error:', sql.ISOLATION_LEVEL.SERIALIZABLE, 'triton coastal locations', failedRows)
  } else {
