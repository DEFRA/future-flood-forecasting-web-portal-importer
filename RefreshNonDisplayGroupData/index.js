...
module.exports = async function (context, message) {
  ...
  await doInTransaction(refreshNonDisplayGroupData, context, 'The FLUVIAL_NON_DISPLAY_GROUP_WORKFLOW refresh has failed with the following error:', sql.ISOLATION_LEVEL.SERIALIZABLE)
  // context.done() not requried as the async function returns the desired result, there is no output binding to be activated.
}

async function refreshNonDisplayGroupData (transactionData, context) {
  const preparedStatement = transactionData.preparedStatement
