...
`)
  ...
  const bindings = await JSON.stringify(context.bindingDefinitions)
  bindings.includes(`"direction":"out"`) ? context.bindings.stagedTimeseries = [] : context.log(`No output binding detected.`)
  ...
  for (const index in timeSeriesData) {
    ...
    if (bindings.includes(`"direction":"out"`)) {
