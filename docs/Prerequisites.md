...
* Microsoft Azure service bus queue named **fews-eventcode-queue**  
  ...
  * This queue **must** be configured to forward **dead lettered messages** to the **replay-fews-eventcode-dead-letter-queue**.
...
* Microsoft Azure service bus queue named **fews-import-queue**.
  ...
  * This queue **must** be configured to forward **dead lettered messages** to the **replay-fews-import-dead-letter-queue**.
...
* Microsoft Azure service bus queue named **replay-fews-import-dead-letter-queue**
  * Messages **must** be forwarded to this queue from the **fews-import-queue/$DeadLetterQueue**. Messages placed on this queue are processed by the **ReplayImportFromFews** function; this function places each message back onto the **fews-import-queue** for message replay. This additional named replay queue is used instead of **fews-import-queue/$DeadLetterQueue** because it can be configured to remove expired messages (a dead letter queue cannot).
* Microsoft Azure service bus queue named **fews-service-configuration-update-completed-queue**
  * Messages are placed on this queue when the **ProcessFewsEventode** or **ImportFromFews** function is disabled and all configuration data has been refreshed within a configurable period of time defined by the [SERVICE_CONFIG_UPDATE_DETECTION_LIMIT app setting](./Non-test-settings-and-environment-variables.md). Messages are processed by the **SendServiceConfigurationUpdateNotification** function. Messages are processed by sending a notification to the location identified by the [SERVICE_CONFIGURATION_UPDATE_NOTIFICATION_URL app setting](./Non-test-settings-and-environment-variables.md).
