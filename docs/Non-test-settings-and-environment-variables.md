...
| SQLDB_TRUST_SERVER_CERTIFICATE | [mssql node module](https://www.npmjs.com/package/mssql) boolean determining if server certificates should be trusted. If this is false (the default) certificate issues will prevent connectivity (uses the mssql module default.) This **must** be set to true when a containerised SQL Server instance is used for unit testing. |
...
| CHECK_FOR_TASK_RUN_DATA_AVAILABILITY_DELAY_MILLIS | The number of milliseconds to wait before replaying a message when all filter based data for a task run is not available (defaults to 2000) |
| WAIT_FOR_TASK_RUN_FILTER_DATA_AVAILABILITY_MILLIS | The number of milliseconds to wait before sending messages for filter based data retrieval when all filter based data for a task run is available from at least one core forecasting engine REST API server instance (defaults to 5000) |
| WAIT_FOR_TASK_RUN_PLOT_DATA_AVAILABILITY_MILLIS | The number of milliseconds to wait before sending messages for plot based data retrieval when it cannot be determined if all data for a task run is available (defaults to 15000) |
| MAXIMUM_DELAY_FOR_DATA_AVAILABILITY_AFTER_TASK_RUN_COMPLETION_MILLIS | The maximum number of milliseconds after task run completion for which checks should be made concerning the availability of task run data from the core forecasting engine REST API before data retrieval is attempted for the task run (defaults to 120000) |
| CHECK_FOR_TASK_RUN_MISSING_EVENTS_DELAY_MILLIS | The number of milliseconds to wait before replaying a message when missing events are detected in timeseries data received from the core forecasting engine REST API (defaults to 30000). A custom setting **must** be applied with caution to avoid risk of core forecasting engine REST API overload. |
| PAUSE_BEFORE_POTENTIAL_MESSAGE_REPLAY_MILLIS | The number of milliseconds to wait before attempting message replay after a transaction failure (defaults to 5000) |
| PAUSE_BEFORE_PROPAGATING_MESSAGE_PUBLICATION_ERROR_MILLIS | The number of milliseconds to wait before propagating a message publication failure (defaults to 2000) |
