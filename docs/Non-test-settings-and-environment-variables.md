...
| SERVICE_CONFIG_UPDATE_DETECTION_LIMIT | The maximum number of seconds since all workflow related CSVs have been updated for a core forecasting engine service configuration update to be detected (defaults to 300). Detection causes replay of failed messages not caused by workflow CSV related configuration errors. |
